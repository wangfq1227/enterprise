            ...
            <div class="left-box">
                ...
                <ul>
                    <c:forEach var="item" items="<%=SystemManage.getInstance().getArticleCategory()%>">
                        <c:if test="${fn:startsWith(item.code,'Business')}">
                            <li <c:if test="${!empty code && code eq item.code}">class="active"</c:if> ><a href="<%=path%>/article/business/${item.code}"><i></i>${item.catename}</a></li>
                        </c:if>
                    </c:forEach>
                </ul>
            </div>
                ...
                <c:forEach var="item" items="${pager.list}">
                    <div>
                        ...
                        <div class="article_item">
                            <h2><a href="<%=path%>/article/business/${item.id}">${item.title}</a></h2>
                            <a href="article/business/${item.id}"><span>${item.description}…</span></a>
                        </div>
                    </div>
                </c:forEach>
            </div>
